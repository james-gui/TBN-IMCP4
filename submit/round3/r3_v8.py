...
CFG = {
    ...
    "profit_exit":         8,     # exit short after this gain (VEV fell this much)
    "loss_exit":           8,     # stop-loss: exit short after this loss (VEV rose this much)
    "cooldown_ticks":      15000, # ticks to wait after a stop before re-entering
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if VEV in state.order_depths and vev_ema is not None:
            ...
            if signal_active and vev_entry_price is not None:
                # Short trade: profit when VEV falls
                price_gain = vev_entry_price - fv
                ...
                if hit_profit or hit_stop:
                    ...
                    otm_signal_ts   = -(10 ** 9)
                    if hit_stop:
                        # Cooldown: don't re-enter until the reversal has exhausted
                        cooldown_until = t + CFG["cooldown_ticks"]

            if signal_active and od.buy_orders:
                if vev_entry_price is None:
                    vev_entry_price = fv
                target_pos = -round(LIMITS[VEV] * size_frac)  # negative = short
                orders: List[Order] = []
                for bid_px in sorted(od.buy_orders.keys(), reverse=True):
                    if pos <= target_pos:
                        break
        ...
        # ── Trade Vouchers (signal SHORT — inverse of insider) ────────────────
        if vev_ema is not None:
            for voucher in VOUCHERS:
                ...
                # Signal inverse: short the OTM strikes the insider bought
                if signal_active and voucher in otm_bought:
                    target_pos = -round(LIMITS[voucher] * size_frac)  # negative = short
                    for bid_px in sorted(od.buy_orders.keys(), reverse=True):
                        if pos <= target_pos:
                            break
                        qty = min(od.buy_orders[bid_px], pos - target_pos)
                ...
                # Signal gone — buy back any short position at best ask
                if pos < 0 and od.sell_orders:
                    best_ask = min(od.sell_orders)
                    orders.append(Order(voucher, best_ask, -pos))
                result[voucher] = orders
