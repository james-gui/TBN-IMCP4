...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        # ── Detect OTM cross-strike signal ────────────────────────────────────
        # Fingerprint: ≥3 OTM vouchers have buy trades in a single tick
        otm_bought: set = set()
        ...
        if HYDROGEL in state.order_depths and hydrogel_ema is not None:
            ...
            result[HYDROGEL] = _mm_orders(
                HYDROGEL, state.order_depths[HYDROGEL], pos, hydrogel_ema, LIMITS[HYDROGEL]
            )
        ...
        if VEV in state.order_depths and vev_ema is not None:
            ...
            if signal_active:
                ...
                if price_gain >= CFG["profit_exit"] or signal_age > CFG["signal_exit_ticks"]:
                    signal_active = False
        ...
        if vev_ema is not None:
            for voucher in VOUCHERS:
                ...
                # Skip ITM options: delta~1, no edge over trading VEV directly
                if K <= vev_ema:
                    result[voucher] = []
                    continue

                # BS arbitrage on OTM only: take when clearly mispriced vs fair value
