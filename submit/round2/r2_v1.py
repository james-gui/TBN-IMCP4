...
# ── Osmium config ──────────────────────────────────────────────────────────────
OSM_CFG = {
    "make_width":      2,     # post at wall_mid ± 2
    "take_width":      1,     # lift/hit anything within 1 tick of wall_mid
    "order_size":      20,
    "inv_limit":       30,    # start skewing above this
    "inv_hard":        60,    # hard cap on passive size reduction
    "ema_alpha":       0.15,  # smoothing on wall_mid for stability
}

# ── Root config ────────────────────────────────────────────────────────────────
ROOT_CFG = {
    "burst_min_len":   3,     # need this many same qty+px trades to confirm a burst
    "buy_offset":      1,     # buy passively at floor_price + offset (maker)
    "take_offset":     2,     # take aggressively if ask <= floor_price + take_offset
}
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        # Root state
        # burst_px:    current pinned floor price
        # burst_qty:   qty of current burst
        # burst_count: consecutive trades at (burst_qty, burst_px)
        ...
        for t in root_trades:
            ...
            if qty == root_burst_qty and px == root_burst_px:
                ...
                # New qty+price combo — start fresh
                root_burst_qty   = qty
                root_burst_px    = px
                root_burst_count = 1

        # Confirmed floor = burst has been running for >= burst_min_len ticks
        ...
        if "INTARIAN_PEPPER_ROOT" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                if floor_price is not None and buy_cap > 0:
                    # ── 1. Aggressive take: lift any ask at or below floor + take_offset ──
                    # The floor bot is absorbing sell pressure; anything near the floor
                    # is cheap relative to where price is going next.
                    take_limit = floor_price + ROOT_CFG["take_offset"]
                    ...
                    # ── 2. Passive bid: sit just above floor to get filled by sellers ──
                    # Post at floor_price + buy_offset (maker, earns the fee rebate)
                    if buy_cap > 0:
                        our_bid = floor_price + ROOT_CFG["buy_offset"]
                        # Don't post above best_ask (would cross the spread)
                        our_bid = min(our_bid, best_ask - 1)
                        if our_bid > best_bid and our_bid < best_ask:
                            orders.append(Order("INTARIAN_PEPPER_ROOT", our_bid, buy_cap))

                elif buy_cap > 0 and not floor_confirmed:
                    # No confirmed floor yet — fall back to best bid + 1 passive bid
                    # to accumulate position while we wait for signal
                    our_bid = best_bid + 1
                    if our_bid < best_ask:
                        orders.append(Order("INTARIAN_PEPPER_ROOT", our_bid, buy_cap))
        ...
        # ── Trade Osmium ──────────────────────────────────────────────────────
        if "ASH_COATED_OSMIUM" in state.order_depths and osm_ema is not None:
            od  = state.order_depths["ASH_COATED_OSMIUM"]
            ...
            if od.buy_orders and od.sell_orders:
                ...
                # ── 2. Make: post at wall_mid ± make_width with inventory skew ──
                inv_ratio = pos / POSITION_LIMIT  # in [-1, 1]
                skew = -round(inv_ratio * 2)      # shift quotes by up to 2 ticks against inventory
                ...
                if our_bid >= our_ask:
                    ...
                    # Size: scale down as inventory approaches hard limit
                    base = OSM_CFG["order_size"]
                    inv_limit = OSM_CFG["inv_limit"]
                    inv_hard  = OSM_CFG["inv_hard"]

                    bid_size = base
                    ask_size = base
                    if pos > inv_limit:
                        scale    = 1.0 - (pos - inv_limit) / max(inv_hard - inv_limit, 1)
