...
OSMIUM_CFG = {
    ...
    "inventory_hard":  50,
    "ema_alpha":       0.08,
    "skew_factor":     3.0,   # fv shifts up to this many ticks to attract balancing flow
}
...
class Trader:
    ...
    def _trade_osmium(self, order_depth: OrderDepth, pos: int, ema_state: Dict[str, float]) -> List[Order]:
        ...
        # Shift fv to attract flow that reduces inventory.
        # Long → fv shifts down (our ask gets cheaper, easier to sell).
        # Short → fv shifts up (our bid gets higher, easier to buy).
        fv = ema_state[product] + (-pos / POSITION_LIMIT) * OSMIUM_CFG["skew_factor"]
        ...
        # Take buys: ask looks cheap relative to fair value
        for ask_px in sorted(order_depth.sell_orders.keys()):
            if remaining_buy <= 0:
                break
        ...
        # Take sells: bid looks rich relative to fair value
        for bid_px in sorted(order_depth.buy_orders.keys(), reverse=True):
            if remaining_sell <= 0:
                break
