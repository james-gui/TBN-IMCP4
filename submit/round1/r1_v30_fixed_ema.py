...
class Trader:
    ...
    def _trade_osmium(self, order_depth: OrderDepth, pos: int, ema_state: Dict[str, float]) -> List[Order]:
        ...
        # Take buys: ask looks cheap vs fv AND ask is below current mid
        # (prevents buying into up-spikes where EMA lags above true fair value)
        for ask_px in sorted(order_depth.sell_orders.keys()):
            if remaining_buy <= 0:
                break
            if ask_px <= fv - take_width and ask_px < current_mid:
        ...
        # Take sells: bid looks rich vs fv AND bid is above current mid
        # (prevents selling into down-spikes where EMA lags below true fair value)
        for bid_px in sorted(order_depth.buy_orders.keys(), reverse=True):
            if remaining_sell <= 0:
                break
            if bid_px >= fv + take_width and bid_px > current_mid:
