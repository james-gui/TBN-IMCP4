...
class Trader:
    ...
    # ── Osmium: Market-making (wall-mid approach) ────────────────────
    def _trade_osmium(self, order_depth: OrderDepth, pos: int, ema_state: Dict[str, float]) -> List[Order]:
        ...
        # Fair value = current wall mid (no EMA lag)
        ...
        # Takes: anything better than fair value
        for ask_px in sorted(order_depth.sell_orders.keys()):
            if remaining_buy <= 0:
                break
        ...
        # Passive quotes: improve the book around fair value
