...
# ── Strategy: r1_v4 ───────────────────────────────────────────────────────────
#
# INTARIAN_PEPPER_ROOT: intara_r1_v3 logic.
#   Calibrates root_base from first observed mid, then tracks drift as
#   fair = root_base + timestamp/1000. Buys up to fair+8, passive bid near
#   fair, only sells above fair+20. Target: hold max long.
#
# ASH_COATED_OSMIUM: pepper_hold (mmfinetuned) logic.
...
class Trader:
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        # Calibrate root_base once from first observed mid
        if root_base is None and "INTARIAN_PEPPER_ROOT" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                logger.print(f"[ROOT] calibrated root_base={root_base:.2f}")
    ...
    def _trade_root(self, state: TradingState, order_depth: OrderDepth, pos: int, root_base: float) -> List[Order]:
        ...
        # Aggressively accumulate — lift any ask up to fair + aggressive_buy_offset
        ...
        for ask_px in sorted(order_depth.sell_orders.keys()):
            ...
            if ask_px <= acceptable_buy:
                ...
                buy_cap -= qty
                pos     += qty
                logger.print(f"[ROOT] TAKE {qty}@{ask_px} fair={fair:.2f}")
            else:
                break
        ...
        if buy_cap > 0 and pos < ROOT_CFG["target_position"]:
            ...
            if our_bid < best_ask:
                ...
                if qty > 0:
                    ...
                    logger.print(f"[ROOT] PASSIVE bid {qty}@{our_bid}")

        # Only sell into very rich bids
        ...
        for bid_px in sorted(order_depth.buy_orders.keys(), reverse=True):
            ...
            if bid_px >= expensive_sell:
                ...
                sell_cap -= qty
                logger.print(f"[ROOT] SELL {qty}@{bid_px} fair={fair:.2f}")
            else:
                break
