...
class PayoffModel:
    ...
    def all_choices_against(self, speed_strategy: list[float]) -> list[AllocationChoice]:
        _, choices = self.expected_choices(speed_strategy)
        return choices

    def top_robust_allocations(
        self, speed_strategies: list[list[float]], limit: int = 5
    ) -> list[RobustAllocation]:
        cdfs_per_strategy = []
        for speed_strategy in speed_strategies:
            cdf_values = []
            cumulative = 0.0
            for share in speed_strategy:
                cumulative += share
                cdf_values.append(cumulative)
            cdfs_per_strategy.append(cdf_values)

        ranking: list[RobustAllocation] = []
        for speed in SPEED_CHOICES:
            products = self._products[speed]
            costs = self._costs[speed]
            for idx, product in enumerate(products):
                pnls = [
                    product * self.speed_multiplier_from_cdf(cdf_values[speed]) - costs[idx]
                    for cdf_values in cdfs_per_strategy
                ]
                ranking.append(
                    RobustAllocation(
                        research=self._research[speed][idx],
                        scale=self._scale[speed][idx],
                        speed=speed,
        ...
        ranking.sort(key=lambda item: (item.worst_pnl, item.average_pnl), reverse=True)
        return ranking[:limit]
...
def point_mass_strategy(speed: int) -> list[float]:
    strategy = [0.0] * NUM_SPEED_CHOICES
    strategy[speed] = 1.0
    return strategy


def band_strategy(start: int, end: int, peak: float | None = None) -> list[float]:
    strategy = [0.0] * NUM_SPEED_CHOICES
    if peak is None:
        ...
        for speed in range(start, end + 1):
            strategy[speed] = 1.0 / (1.0 + abs(speed - peak))
...
def low_speed_pool_strategy() -> list[float]:
    return weighted_average(
        [point_mass_strategy(0), point_mass_strategy(1), point_mass_strategy(2)],
        [0.75, 0.20, 0.05],
    )


def competitive_band_strategy() -> list[float]:
    return band_strategy(33, 38, peak=35.5)


def greedy_band_strategy() -> list[float]:
    # Greedy players shade slightly above the consensus speed band to buy rank.
    return band_strategy(36, 45, peak=39.0)


def build_empirical_market(
    low_share: float,
    competitive_share: float,
    greedy_share: float,
    random_share: float,
) -> list[float]:
    total = low_share + competitive_share + greedy_share + random_share
    if abs(total - 1.0) > 1e-9:
        raise ValueError("Empirical market weights must sum to 1.")
    return weighted_average(
        [
            low_speed_pool_strategy(),
            competitive_band_strategy(),
            greedy_band_strategy(),
            intuitive_noise_strategy(),
        ],
        [low_share, competitive_share, greedy_share, random_share],
    )


def default_empirical_scenarios() -> list[EmpiricalScenario]:
    return [
        EmpiricalScenario(
            name="Empirical low-speed heavy",
            market_strategy=build_empirical_market(0.55, 0.25, 0.15, 0.05),
            notes="Most opponents sit at minimal speed; the 33-38 band is present but secondary.",
        ),
        EmpiricalScenario(
            name="Empirical split pools",
            market_strategy=build_empirical_market(0.40, 0.40, 0.15, 0.05),
            notes="The low-speed pool and the 33-38 pool are both large, with a modest greedy tail.",
        ),
        EmpiricalScenario(
            name="Empirical greedy edge",
            market_strategy=build_empirical_market(0.30, 0.35, 0.30, 0.05),
            notes="A larger group shades above the consensus speed band to win rank.",
        ),
    ]
...
def describe_empirical_scenario(
    model: PayoffModel,
...
def describe_robust_allocations(
    model: PayoffModel,
    empirical_scenarios: list[EmpiricalScenario],
    limit: int = 5,
) -> str:
    robust = model.top_robust_allocations(
        [scenario.market_strategy for scenario in empirical_scenarios],
        limit=limit,
    )
    lines = [
        "Robust empirical response",
        "  objective: maximize worst-case PnL across the empirical pool scenarios",
        "  top allocations:",
    ]
    for index, choice in enumerate(robust, start=1):
        lines.append(
            "    "
            f"{index}. R={choice.research} S={choice.scale} V={choice.speed} | "
            f"worst={choice.worst_pnl:,.2f} | avg={choice.average_pnl:,.2f} | "
            f"best={choice.best_pnl:,.2f}"
        )
    return "\n".join(lines)
...
def main() -> None:
    ...
    empirical_scenarios = default_empirical_scenarios()
    strategic_results = [
    ...
    for scenario in empirical_scenarios:
        print(describe_empirical_scenario(model, scenario, limit=args.top_k))
        print()

    print(describe_robust_allocations(model, empirical_scenarios, limit=args.top_k))
    print()
